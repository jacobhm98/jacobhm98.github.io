+++
title = 'Thoughts on Agentic Development'
...
When kickstarting a task (always in plan mode), I would start off by giving the model initial instructions and acceptance requirements. It would then kick-off its exploration phase -- read through the code base, and by any other means find necessary context pertaining to the task. When complete, the model presents a detailed plan for review. This plan contains key architectural decisions, which files will be modified and how, which automated steps the model will take for automated verification steps at the end, etc...

I usually still have a few back and forth's with the model at this stage. I will tell it to use a slightly different pattern for accomplishing a specific task, or ask it to organize the code slightly more consistently with the code base, or sometimes larger course corrections are necessary. Generally, the larger the task, the more work I need to put into getting the plan correct at this stage.
...
In practice, I have found that 4-5 parallel agents is a good number. Any more and I find myself not giving each task its proper dues, any less and I'm spending time rolling my thumbs waiting for an agent to complete.
...
2. Write CLAUDE.md/AGENTS.md files pointing your agent to which systems contains which data, and instructions for how to access that data.
3. Use monorepos!
...
With agents, especially with monorepos, you need to facilitate the parallel, but independent, modification of your codebase. Git worktrees are the common way to accomplish this, they check out your repository into a separate directory, allowing "containerized" changes.
...
I think the second point here is really key. If we think back to how the development process has changed, writing code is no longer the bottle neck. Instead, the planning and verification of changes are.
...
The last one was built by yours truly, and is a tool I use pretty much every day, check it out! I'm particularly proud of my solution to problem number 2.
...
If you primarily cared about solving interesting problems (through code or otherwise), a worry that I had was that as these models get better and better, there won't be any interesting problems left to solve.
...
Oftentimes, you'll have two or more solutions that are all entirely technically correct. But the particulars of your situation: how the solution will interact with the rest of your system; _what_ your users truly care about; how you anticipate the needs of your company will develop in the future; what the shape of the team maintaining the system long-term will be; etc..., will nudge the correctness of each alternative _for you_ in one direction or another.

Making the choice between one correct solution and another, is where I find more and more of my mental capacity going. This is still technical work -- I need to understand each option completely, I need to have a complete understanding of the system I'm working on, and I need to have a mental model of how I think the system is likely to develop in the future in order to make the correct choice.

#### Stewarding and planning

I am also still critically involved in stewarding the system we have built. This is, and has always been, part of the role of a software engineer. As time passes, your user profile or count changes, or your data consumption and processing patterns change, and as a result your system is placed under novel strain. Operating such a system means continuous introspection in the form of metrics and alerts, and making the correct assessment in which parts need to be investigated, made more efficient, or had more resources apportioned to.
...
The other, obvious benefit, the reason we all do this in the first place, is that you will be able to deliver useful software much more rapidly than in the past.

Again, looking at my personal experience. I want to caveat this by saying that I am working on a startup with 2 other people. This means that there's exceptionally little in the way of bureaucracy or process, and most of my time is spent building, shipping, fixing, and improving software. For various reasons, most of them valid, this would not be the case at an established company.

If I take a typical "jira ticket sized" task (create a new API handler with wiring for a specific set of resources, for example), I'd probably get through one in about 5 hours of heads-down work.
...
All this to say, I see the current crop of agents to be an immensely powerful _tool_ to be wielded in the arsenal of a competent human software engineer.

The bottlenecks for complete autonomy are in my mind fundamentally incompatible with the current form of LLM's. We need on the job learning, human-to-human understanding, and an interface into the real world's problems in order to be great software engineers. LLM's are bottle-necked by all of these.

However, if you embrace LLM's, there has never been a better time to build software. Your productivity will go through the roof, and if done well, your enjoyment of software development will follow.

I don't see this changing until the technology underlying the LLM's changes in some fundamental way to address the above constraints.
